...
function Scatterplot({
  ...
  const xAxis: any = useMemo(() => d3.axisBottom(xScale).ticks(9).tickFormat(d3.format('.2g')), [xScale]);

  const yAxis: any = useMemo(() => d3.axisLeft(yScale).ticks(5).tickFormat(d3.format('.2g')), [yScale]);
  ...
  const draw = useCallback(
      ...
      if (domains.length > 0) {
        ...
        if (type) {
          ...
          d3.select($xaxis.current)
            ...
            .call((g) => g.selectAll('.tick line').attr('stroke-opacity', 0.5).attr('stroke-dasharray', '0,25'));

          d3.select($yaxis.current)
            .call(yAxis.scale(yScale))
            .call((g) => g.select('.domain').remove())
            .call((g) => g.selectAll('.tick:not(:first-of-type) line').attr('stroke-opacity', 0.5).attr('stroke-dasharray', '0,25'));
          // clear
    ...
    [domains.length, drawLines, t.k, render, xAxis, xScale, yAxis, yScale, widthMap, heightMap, drawPoints]
  );
  ...
  const drawLasso = useCallback(
    (polygon: any) => {
      if (chartctx && select) {
        ...
        data.forEach((d) => {
          if (polygon.length > 2 && d3.polygonContains(polygon, [xScale(d[xkey]), yScale(d[ykey])])) {
      ...
      $pointsSelect
        .on('mousemove', (event: any) => {
          if (!select) {
            ...
            if (tooltipData !== null) {
              const pxDat = [~~xScale(tooltipData[dimensions[0]]), ~~yScale(tooltipData[dimensions[1]])];
  ...
  const hullExtents = useMemo(() => extents.map((extentArr) => [extentArr[0].map((d) => xScale(d)), extentArr[1].map((d) => yScale(d))]), [
    extents,
    xScale,
    yScale,
  ]);
      ...
      <div className="container">
        <svg width={width} height={height}>
          ...
          <g transform={`translate(${margin.l},${margin.t})`}>
            ...
            <text className={styles.label} transform="rotate(-90)" x={(-heightMap * 12) / 13} dy="20">
              {yaxis.title}
            </text>
            {/* {
              grids.map((gridRow, i) => 
            ...
            <g clipPath="url(#myClip)">
              ...
              {render ||
                (domains.length > 0 &&
                  data.map((dat, i) => (
                    <circle key={i} cx={xScale(dat.dimensions[0])} cy={yScale(dat.dimensions[1])} r={3} fill={pointColor(dat.label)} />
                  )))}
            </g>
          </g>
        </svg>
